// Big O notation is a way to measure and analyze the efficiency of algorithms in terms of their time and space complexity. Time complexity refers to how fast an algorithm runs as the size of the input increases, while space complexity refers to how much additional memory is required to run the algorithm.

// In Big O notation, we use the notation O(f(n)) to represent the upper bound of the number of simple operations a computer has to do, as n (the size of the input) increases. This means that the algorithm's running time is eventually less than a constant times f(n), as n gets larger. The function f(n) could be linear (f(n) = n), quadratic (f(n) = n^2), constant (f(n) = 1), or something entirely different.
...
// When it comes to space complexity, we look at the additional memory that an algorithm needs to allocate to run, not including the space taken by the inputs. For example, the auxiliary space complexity refers to the space required by the algorithm itself.
...
// It's also important to remember that logarithms are commonly used in searching and sorting algorithms, as well as recursion algorithms, but typically for space complexity rather than time complexity.

// To see these concepts in action, we can consider a few examples of functions with different time and space complexities:

// Example 1: addUpTo(n)
// This function takes in a number n and returns the sum of all numbers from 1 up to and including n. The time complexity of this function is O(n), as the loop runs n times. The space complexity is O(1), as the function only uses a single variable to store the total sum.

// Example 2: double(arr)
// This function takes in an array arr and returns a new array where each element is double the corresponding element in the input array. The time complexity of this function is O(n), as the loop runs n times (where n is the length of the input array). The space complexity is also O(n), as the function creates a new array with n elements to store the doubled values.
